...
from martian import openai, util
...
def get_headers(openai_api_key: str) -> str:
    martian_api_key = util.default_api_key()
    res = f"Bearer {martian_api_key}"
    if openai_api_key:
        res += f", Bearer {openai_api_key}"
    return res


api_key_to_header = (
    lambda api, key: {
        "Authorization": get_headers(key),
    }
...
def get_object_classes():
    ...
    from martian.openai.object_classes import OBJECT_CLASSES
...
def default_api_key() -> str:
    if openai.api_key_path:
        with open(openai.api_key_path, "rt") as k:
            ...
            return api_key
    return openai.api_key  # NOTE: Martian Change: We allow this value to be `None``.
